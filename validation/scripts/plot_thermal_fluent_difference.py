#!/usr/bin/env python3
"""Generate thermal Fluent difference comparison plots."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate thermal Fluent difference plots")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output dir (default: <case>/out/thermal/fluent_comparison/difference)",
    )
    ...
    out_dir = args.output_dir or (case.output_dir / "thermal" / "fluent_comparison" / "difference")
    out_dir.mkdir(parents=True, exist_ok=True)

    plot_thermal_fluent_envelope_difference(
        comp,
        title=f"Temperature difference envelope - Thermal vs Fluent - {case.name}",
        output_path=out_dir / "thermal_fluent_envelope_diff.png",
    )
    plt.close("all")
    ...
    for side, thermal_res, fluent_field in [
        ...
        plot_thermal_fluent_contour_difference(
            thermal_res,
            fluent_field,
            title=f"{side.capitalize()} temperature difference - Thermal vs Fluent - {case.name}",
            output_path=out_dir / f"thermal_fluent_contour_diff_{side}.png",
        )
        plt.close("all")
        ...
        plot_thermal_fluent_contour_normalized_difference(
            thermal_res,
            fluent_field,
            title=f"{side.capitalize()} normalized temperature difference - Thermal vs Fluent - {case.name}",
            output_path=out_dir / f"thermal_fluent_contour_normalized_diff_{side}.png",
        )
        plt.close("all")

    if args.show_plots:
        plt.show()
    print(f"Saved thermal difference plots to: {out_dir}")
    return 0
