#!/usr/bin/env python3
"""Generate thermal wall line plots (s-Tw and s-h) vs Fluent."""
...
from visualization.thermal_fluent_comparison_plots import plot_thermal_wall_line_comparison
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate thermal wall line Fluent comparison plots")
    ...
    parser.add_argument(
        "--quantities",
        nargs="+",
        default=["wall_temperature", "heat_transfer_coeff"],
        help="Wall quantities to plot",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output dir (default: <case>/out/thermal/fluent_comparison/wall_lines)",
    )
    ...
    out_dir = args.output_dir or (case.output_dir / "thermal" / "fluent_comparison" / "wall_lines")
    ...
    for qty in args.quantities:
        plot_thermal_wall_line_comparison(
            comp,
            quantity=qty,
            title=f"{qty} line comparison - Thermal vs Fluent - {case.name}",
            output_path=out_dir / f"thermal_wall_{qty}_line.png",
        )
        plt.close("all")

    if args.show_plots:
        plt.show()
    print(f"Saved thermal wall line plots to: {out_dir}")
    return 0
